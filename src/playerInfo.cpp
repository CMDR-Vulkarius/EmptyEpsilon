...
#include "screens/extra/repairsScreen.h"
...
void PlayerInfo::spawnUI(int monitor_index, RenderLayer* render_layer)
{
    ...
    else
    {
        ...
        if (crew_position[repairsScreen] & (1 << monitor_index))
            screen->addStationTab(new RepairsScreen(container), repairsScreen, getCrewPositionName(repairsScreen), getCrewPositionIcon(repairsScreen));
...
string getCrewPositionName(ECrewPosition position)
{
    ...
    case repairsScreen: return tr("station","Repairs");
...
template<> void convert<ECrewPosition>::param(lua_State* L, int& idx, ECrewPosition& cp)
{
    ...
    //extras
        ...
        cp = shipLog;
    else if (str == "repairs" || str == "repairsscreen")
        cp = repairsScreen;
    else
