-- Name: ECM Patrol
-- Description: Patrol between three stations in the Delta quadrant aboard the carrier Saipan. Launch fighters as needed to defend against enemy waves.
---
--- Version 1 (Based on Patrol Duty V8)
-- Type: Mission
...
function init()
	...
	-- Spawn Saipan carrier + docked fighter near Asimov station
	local startX = 3000
	local startY = 0
	
	playerCarrier = PlayerSpaceship():setFaction("Human Navy")
	playerCarrier:setTemplate("Saipan")
	playerCarrier:setPosition(startX, startY)
	playerCarrier:setCallSign("Saipan")
	
	playerFighter = PlayerSpaceship():setFaction("Human Navy")
	playerFighter:setTemplate("Player Fighter")
	playerFighter:setPosition(startX, startY)  -- Will be inside carrier after docking
	playerFighter:setJumpDrive(false)
	playerFighter:setWarpDrive(false)
	playerFighter:commandDock(playerCarrier)
